# -*- coding: utf-8 -*- 
"""
Script per estrarre dati bibliometrici da Scopus usando i DOI.
Autori: Francesco & Mariella

Note:
- Il mapping (UNI_MAPPING e UNI_REGION) è quello “base” (in inglese) che cerca nel testo le stringhe indicate.
- Per il calcolo del numero medio di autori per riferimento viene usato il metodo che suddivide la stringa degli autori in base a ";".
- I dati vengono aggregati per regione e salvati in file Excel; inoltre, vengono creati dei plot.
- In aggiunta, vengono salvati file Excel separati per università pubbliche e private e viene realizzato un boxplot comparativo.
- Nei file finali è incluso anche l'anno di pubblicazione e il Source Title.
"""
...
if not CONFIG_PATH.exists():
    ...
    config["Authentication"] = {
        "APIKey": "ee1866f96d6af76a750def9a35c21d6a,67c95731750e364ef8c277caefa27536,c65f4036ada645d2f2f317e015b41d93,"
                  "fd46a92d839d6b9ab84aa9cd531d94c2,67a759516292a895f74b9a7f5d9aeecb,82801518de34b2e8e88b96d57c3fc49d,"
                  "068b755621c6907f2758f275a07fffc9,b2dc4eae2835b79ce18880bba0740580,b5c27402c3a7aea3a5f94596857cc96e,"
                  "fe2187fcb0f813ddc0b694e09d6eeda2"
    }
    
    ##Replace the previous APIKeys and paths for the pybliometrics modules with 
    #the ones used by the user after having installed successfully pybliometrics
...
# ==================== INPUT FOLDER ====================
INPUT_DIR = Path(r"G:\Drive condivisi\fair index paper SCoSC\science of science\dati\Neuroscience files")
os.chdir(INPUT_DIR)

# ==================== OUTPUT FOLDER ====================
OUTPUT_DIR = Path(r"G:\Drive condivisi\fair index paper SCoSC\science of science\risultati\all italian papers nuova selezione")
...
UNI_MAPPING = {
    ...
    # Group 3: Central Italy Universities
    ...
    # Group 3: Central Italy (Rome and its surroundings)
    ...
    # Group 5: Southern Italy and Islands universities
...
agg_region_data = {
    "Northern Italy": {
        ...
        'Italian University First': [], 'Italian University Last': [], 'Italian Author': [],
...
if not excel_files:
    # I file di input devono contenere le colonne: DOI, Authors, Affiliations, Publication Year, Source Title
    excel_files = [f for f in INPUT_DIR.glob("*.xlsx") if not f.name.startswith("authors_number_per_reference_")]
...
required_columns = [
    "DOI", "Authors", "Affiliations", "Publication Year",
    "Source Title", "Document Type", "WoS Categories", "Reprint Addresses"
]
...
for file in excel_files:
    ...
    for i, row in papers_references.iterrows():
        ...
        if ref_data:
            ...
            # qui reinserisco la concatenazione
            ...
            agg_region_data[region]['Italian Author'].append(role)
            ...
            # in caso di fallimento, appendo un solo None per ciascun campo
            ...
            agg_region_data[region]['Italian Author'].append(None)
...
# Salva i file Excel per ciascuna regione
for reg, data_dict in agg_region_data.items():
    if not data_dict['Paper DOI']:
        print(f"ℹ️ No data for {reg}, no file saved.")
        continue
    ...
    print(f"✅ Saved file for the region {reg} in: {output_path}")


df_all = pd.concat([pd.DataFrame(agg_region_data[r]) for r in agg_region_data], ignore_index=True)

mask_bicocca = df_all['Reprint Addresses'].str.contains('Bicocca', case=False, na=False)
mask_milan = df_all['Italian University'] == 'University of Milan'

# Edit only when conditions are true
df_all.loc[mask_bicocca & mask_milan, 'Italian University'] = 'University of Milano-Bicocca'
...
print(f"✅ File Excel con tutti i dati salvato in: {all_output_path}")
